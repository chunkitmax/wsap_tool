...
export default {
  ...
  created() {
    ...
    axios.get('/contacts', { responseType: 'json' }).then(res => {
      this.contacts = res.data.map(contact => {
        if (contact.lastMsg.length > 34) {
          contact.lastMsg = contact.lastMsg.slice(0, 31)+'...'
        }
        return contact
      })
    }, err => {})
